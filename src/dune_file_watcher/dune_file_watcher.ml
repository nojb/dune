...
let create_fsevents ~(scheduler : Scheduler.t) =
    ...
    Fsevents.create ~paths ~latency:0.2 ~f:(fun _ events ->
        scheduler.thread_safe_send_emit_events_job (fun () ->
            List.map events ~f:(fun event ->
                let path =
                  match Path.of_string (Fsevents.Event.path event) with
                  | (In_source_tree _ | In_build_dir _) as p -> p
                  | External e as p -> (
                    match Path.Source.of_external e with
                    | None -> p
                ...
                if
                  is_special_file_for_inotify_sync path
                  &&
                  match action with
                  | Unknown
                  | Create
  ...
  Fsevents.set_exclusion_paths fsevents
    ~paths:
      (let open Path in
      [ build Build.root
      ; relative (source Source.root) "_esy"
      ; relative (source Source.root) "_opam"
      ]
