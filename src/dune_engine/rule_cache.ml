  ...
  let lookup_impl ~rule_digest ~targets ~env ~build_deps =
    ...
    | Hit (prev_trace, produced_targets) ->
      (* CR-someday aalekseyev: If there's a change at one of the last stages,
         ...
         later stages). *)
      let open Fiber.O in
      let check_deps deps_trace error build_mode =
        let deps, old_digest = deps_trace in
      ...
      let* check_needed_deps =
        check_deps
          ...
          (Action_intf.Exec.Lazy Loc.none)
      in
