...
let print_rule_ninja ppf (rule : Dune_engine.Reflection.Rule.t) =
  let pp_to_string pp =
    ...
    Buffer.contents buf
  in
  let has_hash s = String.contains s '#' in
  let output_paths =
    Filename.Set.union rule.targets.files rule.targets.dirs
    |> Filename.Set.to_list_map ~f:(fun basename ->
      Path.Build.relative rule.targets.root basename |> Path.build)
  ...
  in
  let action =
    let cleanup = List.map output_paths ~f:(fun p -> Action.For_shell.Remove_tree p) in
    Action.For_shell.Progn
      (cleanup
       @ [ Mkdir (Path.to_string (Path.build rule.targets.root))
         ; Action.for_shell rule.action
  ...
  in
  let dep_paths = Path.Set.to_list rule.expanded_deps |> List.map ~f:Path.to_string in
  if List.exists output_paths ~f:has_hash || List.exists dep_paths ~f:has_hash
  then Format.fprintf ppf "# skipped rule with '#' in paths\n\n"
  else (
    let outputs = List.map output_paths ~f:ninja_escape_path in
    let deps = List.map dep_paths ~f:ninja_escape_path in
    let command = Action_to_sh.pp action |> pp_to_string |> ninja_escape_value in
    Format.fprintf
      ppf
      "build %s: dune_rule%s\n  cmd = %s\n\n"
      (String.concat outputs ~sep:" ")
      (if List.is_empty deps then "" else " " ^ String.concat deps ~sep:" ")
      command)
;;
...
module Syntax = struct
  ...
  let term =
    ...
    let+ makefile = Arg.(value & flag & info [ "m"; "makefile" ] ~doc:(Some doc))
    and+ ninja =
      Arg.(
        value
        & flag
        & info [ "n"; "ninja" ] ~doc:(Some "Output the rules in Ninja syntax."))
    in
    match makefile, ninja with
    | true, true ->
      User_error.raise [ Pp.text "Cannot use both --makefile and --ninja." ]
    | true, false -> Makefile
    | false, true -> Ninja
    | false, false -> Sexp
  ;;
  ...
  let requires_recursive = function
    | Ninja -> true
    | Makefile
    | Sexp -> false
  ;;
  ...
  let print_rules syntax ppf rules =
    ...
    (match syntax with
     | Ninja ->
       Format.fprintf ppf "# Generated by dune rules --ninja\n";
       Format.fprintf ppf "ninja_required_version = 1.3\n\n";
       Format.fprintf ppf "rule dune_rule\n  command = $cmd\n\n"
     | Makefile
     | Sexp -> ());
...
let term =
  ...
  Scheduler_setup.go_with_rpc_server ~common ~config (fun () ->
    ...
    build_exn (fun () ->
      ...
      in
      let recursive = recursive || Syntax.requires_recursive syntax in
